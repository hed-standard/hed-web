...
class TestEventServices(ServicesTest):
    ...
    def test_get_services(self):
        ...
        print(f"Error report: [{response_data.get('error_type')}] {response_data.get('error_msg')}")

        # Print out the results if available
        if 'results' in response_data and response_data['results']:
            results = response_data['results']
            print(f"[{response_data.get('service')}] status {results.get('msg_category')}: {results.get('msg')}")
            print(f"Return data:\n{results.get('data')}")
