...
class Test(TestWebBase):
    ...
    def get_spread_proc(
        ...
        if spread_file:
            spread_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), spread_file
            )
    ...
    def test_set_input_from_spreadsheets_form(self):
        with self.app.test:
            ...
            self.assertIsInstance(
                spread_proc.schema, HedSchema, "should have a HED schema"
            )
            self.assertEqual(
                spread_proc.command, bc.COMMAND_VALIDATE, "should have a command"
            )
            self.assertEqual(
                spread_proc.worksheet, "LKT 8HED3", "should have a sheet_name name"
            )
    ...
    def test_set_input_from_spreadsheets_form_other(self):
        with self.app.test:
            ...
            schema_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "data/HED8.2.0.xml"
            )
            with open(spreadsheet_path, "rb") as fp:
    ...
    def test_spreadsheets_process_validate_invalid(self):
        with self.app.app_context():
            spread_proc = self.get_spread_proc(
                "data/ExcelMultipleSheets.xlsx", worksheet="LKT Events", tag_columns=[4]
            )
            ...
            self.assertTrue(
                results["data"], "should return validation issues using HED 8.2.0"
            )

    def test_spreadsheets_validate_valid(self):
        with self.app.app_context():
            spread_proc = self.get_spread_proc(
                "data/ExcelMultipleSheets.xlsx", worksheet="LKT 8HED3A", tag_columns=[4]
            )
            spread_proc.command = bc.COMMAND_VALIDATE
            spread_proc.check_for_warnings = True
            results = spread_proc.process()
            self.assertTrue(
                isinstance(results, dict), "should return a dict when no errors"
            )
            self.assertEqual(
                "success", results["msg_category"], "should return success if validated"
            )
    ...
    def test_spreadsheets_validate_valid_excel(self):
        with self.app.app_context():
            ...
            self.assertFalse(
                results["data"], "should not have a data key when no validation issues"
            )
            self.assertEqual(
                "success", results["msg_category"], "should be success when no errors"
            )
    ...
    def test_spreadsheet_validate_definitions(self):
        spread_proc = self.get_spread_proc(
            "data/spreadsheet_with_defs.tsv", tag_columns=[2]
        )
