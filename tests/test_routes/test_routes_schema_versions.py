...
class Test(TestRouteBase):
    def test_schema_versions(self):
        with self.app.app_context():
            ...
            self.assertEqual(
                200, response.status_code, "The HED version list does not require data"
            )
            versions = response.data
            self.assertTrue(versions, "The returned data is not empty")
            v_dict = json.loads(versions)
            self.assertIsInstance(
                v_dict, dict, "The versions are returned in a dictionary"
            )
    ...
    def test_schemas_version_results1(self):
        with self.app.app_context():
            ...
            response = self.app.test.post(
                "/schema_version", content_type="multipart/form-data", data=data
            )
            self.assertEqual(
                200, response.status_code, "The HED version list does not require data"
            )
