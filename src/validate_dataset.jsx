...
function ValidateDatasetApp() {
  ...
  const getFileCounts = (dataset) => {
    ...
    if (
      !dataset ||
      !dataset.fileAccessor ||
      !dataset.fileAccessor.organizedPaths
    ) {
      return counts;
    }
  ...
  const handleFolderSelect = async (selectedFiles) => {
    ...
    const [createdDataset, issues] = await BidsDataset.create(
      selectedFiles,
      BidsWebAccessor,
    );
    ...
    if (processedIssues.length > 0) {
      ...
      const { json: numJsonFiles, tsv: numTsvFiles } =
        getFileCounts(createdDataset);
      setSuccessMessage(
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const processedSidecarIssues = BidsHedIssue.processIssues(
        issues,
        checkWarnings,
        limitErrors,
      );
      if (processedSidecarIssues.some((issue) => issue.severity === "error")) {
        ...
        setDownloadableErrors(
          BidsHedIssue.processIssues(issues, checkWarnings, false),
        );
        setFatalError(
          "Validation cannot continue due to serious sidecar errors.",
        );
        setIsLoading(false);
        setValidationStatus("");
        return;
      ...
      setSuccessMessage(
        "Preliminary sidecar validation passed. Continuing with TSV validation...",
      );
      ...
      issues = BidsHedIssue.transformToBids([err], {
